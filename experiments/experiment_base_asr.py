...
class ExperimentRunnerBase:
    ...
    def train(self):
        ...
        asr_avg_train_acc = AverageMeter()
        ...
        for epoch in range(self.num_epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(self.train_loader)):
                ...
                if step % self.val_every == 0:
                    ...
                    print('Val acc (audio) = {:.4f}, Val acc (asr) = {:.4f}, Val loss = {:.4f}'.format(val_acc, asr_val_acc, val_loss))
                    if self.visualize:
    ...
    def train_step(self, batch):
        ...
        self.optimizer.step()
        return metrics['loss'], metrics['accuracy'], metrics['asr_accuracy']
    ...
    def val(self):
        ...
        asr_avg_val_acc = AverageMeter() #asr acc

        self.model.eval()
        for batch_idx, batch in enumerate(tqdm(self.val_loader)):
            metrics = self.compute_loss(batch)
            avg_val_acc.update(metrics['correct'].cpu().numpy())
            asr_avg_val_acc.update(metrics['asr_correct'].cpu().numpy())
            avg_val_loss.update([metrics['loss']])
        return avg_val_loss.get(), avg_val_acc.get(), asr_avg_val_acc.get()
...
#         all_asr_embeddings = []
        ...
        for batch_idx, batch in enumerate(tqdm(self.test_loader)):
            ...
            asr_avg_test_acc.update(output['asr_correct'].cpu().numpy())
...
#                             labels=all_true_labels)
        ...
        print('Final test acc (audio) = {:.4f}, final test acc (asr) = {:.4f} test loss = {:.4f}'.format(avg_test_acc.get(), asr_avg_test_acc.get(), avg_test_loss.get()))
        return avg_test_loss.get(), avg_test_acc.get(), asr_avg_test_acc.get()

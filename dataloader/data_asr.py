...
class BaseDataset(Dataset):
    ...
    def load_audio(self, idx):
        ...
        encoding_asr = self.bert_tokenizer.encode_plus(
            df_row['asr_transcript'],
            add_special_tokens=True,
            return_token_type_ids=False,
            return_tensors='pt'
            )
        return fbank_feats, intent, encoding, df_row['transcription'], encoding_asr #df_row['asr_transcript']

    def get_dict(self, fbank_feats, intent, encoding, transcription, encoding_asr, suffix=''):
        ret_dict = {'feats':fbank_feats, 'length':fbank_feats.shape[0] ,'label':intent, #audio
                    'encoded_text':encoding['input_ids'].flatten(), 'text_length':encoding['input_ids'].flatten().shape[0], 'raw_text':transcription, #text
                   'encoded_asr':encoding_asr['input_ids'].flatten(), 'asr_length':encoding_asr['input_ids'].flatten().shape[0]} #asr
...
def triplet_getter(self, idx):
    fbank_feats, intent, encoding, transcription, encoding_asr = self.load_audio(idx)
    ...
    fbank_feats2, intent2, encoding2, transcription2, encoding_asr2 = self.load_audio(pos_idx)
    fbank_feats3, intent3, encoding3, transcription3, encoding_asr3 = self.load_audio(neg_idx)

    dict1 = self.get_dict(fbank_feats, intent, encoding, transcription, encoding_asr)
    dict2 = self.get_dict(fbank_feats2, intent2, encoding2, transcription2, encoding_asr2, suffix='2')
    dict3 = self.get_dict(fbank_feats3, intent3, encoding3, transcription3, encoding_asr3, suffix='3')
...
class FluentSpeechDataset(BaseFluentSpeechDataset):
    ...
    def __getitem__(self, idx):
        fbank_feats, intent, encoding, transcription, encoding_asr = self.load_audio(idx)
        return self.get_dict(fbank_feats, intent, encoding, transcription, encoding_asr)
...
def default_collate_classifier(inputs):
    ...
    encoded_asr = [data['encoded_asr'] for data in inputs]
    asr_lengths = [data['asr_length'] for data in inputs]
        
    
    padded_feats = rnn.pad_sequence(feats, batch_first=True)
    padded_text = rnn.pad_sequence(encoded_text, batch_first=True)
    padded_asr = rnn.pad_sequence(encoded_asr, batch_first=True)
    
    labels = torch.tensor(labels, dtype=torch.long)
    lengths = torch.tensor(lengths, dtype=torch.long)
    text_lengths = torch.tensor(text_lengths, dtype=torch.long)
    asr_lengths = torch.tensor(asr_lengths, dtype=torch.long)
    
    return {'feats':padded_feats, 'length':lengths, 'label':labels, 'encoded_text':padded_text, 'text_length':text_lengths, 'raw_text':raw_text, 'encoded_asr':padded_asr, 'asr_length':asr_lengths}
...
def default_collate_pairwise(inputs):
    ...
    ORIGINAL - we don't use/edit this!
    '''
    feats = [data['feats'] for data in inputs]
    labels = [data['label'] for data in inputs]
    lengths = [data['length'] for data in inputs]
